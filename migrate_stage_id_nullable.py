#!/usr/bin/env python3
"""
Миграция для изменения поля stage_id в таблице field_stage_role_permissions,
чтобы оно могло быть NULL (для правил, действующих на все этапы).
"""
...
def migrate():
    ...
    try:
        ...
        stage_id_column = None
        for col in columns:
            if col[1] == 'stage_id':
                stage_id_column = col
                break
        
        if not stage_id_column:
            print("Колонка stage_id не найдена. Пропускаем миграцию.")
            return
        
        # Если колонка уже nullable (notnull = 0), миграция не нужна
        if stage_id_column[3] == 0:
            print("Колонка stage_id уже nullable. Миграция не требуется.")
            return
        
        print("Начинаем миграцию stage_id в nullable...")
        ...
        # Создаем новую таблицу с nullable stage_id
        cursor.execute("""
